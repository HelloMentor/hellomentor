...
DB.prototype.connect = function(uri) {
	...
	return new Promise(function(resolve, reject) {
		if (_this.db) {
			...
			MongoClient.connect(uri ? uri : config.client.mongodb.defaultUri)
			.then(
				function(database) {
					...
					__this.parentDbObj = database;
					__this.db = database.db(config.client.mongodb.defaultDatabase);
...
DB.prototype.find = function(coll, query) {
	var _this = this;
	
	return new Promise(function (resolve, reject) {
		_this.db.collection(coll, { strict: true }, function(error, collection) {
			if (error) {
				console.error("Could not access collection: " + error.message);
...
DB.prototype.addDocuments = function(coll, docs) {
